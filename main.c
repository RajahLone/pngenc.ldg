...
{
  PngLdgBuffer *buf = (PngLdgBuffer *) png_get_io_ptr(png_ptr);
...
int32_t CDECL pngenc_open(uint32_t width, uint32_t height, uint32_t depth, uint32_t colortype, uint32_t frames_count, uint32_t plays_count)
{
  ...
  if (buffer.data == NULL) { return PNG_ERROR; }
  ...
  if (png_ptr && info_ptr)
  {
    ...
    png_set_write_fn(png_ptr, &buffer, pngldg_write, pngldg_flush);
...
uint8_t* CDECL pngenc_get_filedata() { return buffer.data; }
uint32_t CDECL pngenc_get_filesize() { return buffer.offset; }
